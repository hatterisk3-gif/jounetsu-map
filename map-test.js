...
/** 'prod' | 'water' — 生産管理 / 水管理の表示モード */
let mapMode = 'prod';
let lastDrawnFieldData = null;

function publishHostGlobals_() {
    window.map = map;
...
// ====== ログイン ======
function resolveInitialMapMode_() {
    try {
        const params = new URLSearchParams(window.location.search || '');
        const q = (params.get('mode') || '').toLowerCase();
        if (q === 'water' || q === 'prod') return q;
    } catch (e) {}
    const saved = localStorage.getItem('mapUiMode');
    if (saved === 'water' || saved === 'prod') return saved;
    return 'prod';
}

function applyMapModeChrome_(mode) {
    mapMode = (mode === 'water') ? 'water' : 'prod';
    document.body.classList.remove('map-mode-prod', 'map-mode-water');
    document.body.classList.add(mapMode === 'water' ? 'map-mode-water' : 'map-mode-prod');
    const btnProd = document.getElementById('btnModeProd');
    const btnWater = document.getElementById('btnModeWater');
    if (btnProd) {
        btnProd.className = mapMode === 'prod' ? 'active-prod' : '';
    }
    if (btnWater) {
        btnWater.className = mapMode === 'water' ? 'active-water' : '';
    }
    publishHostGlobals_();
}
...
document.addEventListener('DOMContentLoaded', () => {
    applyMapModeChrome_(resolveInitialMapMode_());
    ...
    if (cached) {
        ...
        requestAnimationFrame(() => {
            try {
                ...
                else if (typeof restoreMapInteractions_ === 'function') restoreMapInteractions_();
...
async function executeLogin(isAuto = false, options = {}) {
    ...
    const startupLoad = isAuto ? window._mapStartupLoading : null;
    ...
    try {
        ...
        if (result.success) {
            ...
            if (fromCache) {
                loadInitData({ background: true }).then(() => {
                    if (mapMode === 'water' && window.WaterMode) window.WaterMode.activate();
                    setTimeout(refreshProductionMapDisplay_, 120);
                });
                return;
            }
            ...
            if (cached) {
                ...
                startupLoad.update({ detail: '保存データを確認しました', current: 2, total: 4 });
            }
        ...
        if (fromCache || isAuto) {
            ...
            if (startupLoad) {
                if (cached || fromCache) startupLoad.done();
                else startupLoad.fail('圃場データの読み込みに失敗しました');
                window._mapStartupLoading = null;
            }
            if (typeof ensureMapGesturesEnabled === 'function') ensureMapGesturesEnabled();
            if (fromCache || cached) loadInitData({ background: true });
        } else {
            document.getElementById('loginScreen').style.display = 'flex';
...
function initMap() {
    ...
    const hasValidCenter = savedLat !== null && savedLng !== null && savedLat !== '' && savedLng !== ''
        && Number.isFinite(lat) && Number.isFinite(lng)
        && lat >= -90 && lat <= 90 && lng >= -180 && lng <= 180;
    let centerPos = hasValidCenter ? { lat, lng } : { lat: 33.91, lng: 134.66 };
    if (!hasValidCenter) {
        localStorage.removeItem('manureMapLat');
        localStorage.removeItem('manureMapLng');
    }
...
function refreshProductionMapDisplay_() {
    ...
    try {
        ...
        if (appLoad) {
            appLoad.update(externalLoad
                ? { detail: '取得データを確認しています', current: 3, total: 4 }
                : { detail: 'データを確認しています', current: 2, total: 3 });
        }
        ...
        if (data && data.polygons) {
            ...
            if (hasCache && incomingCount === 0) {
                ...
                else if (!background && !appLoad && typeof restoreMapInteractions_ === 'function') restoreMapInteractions_();
                return true;
            }
            ...
            if (appLoad) {
                appLoad.update(externalLoad
                    ? { detail: '地図を描画しています', current: 4, total: 4 }
...
function drawPolygons(dataList) {
    if (!Array.isArray(dataList)) {
        throw new Error('圃場データの形式が正しくありません');
    }
    lastDrawnFieldData = dataList;
    ...
    dataList.forEach(pData => {
        ...
        let color;
        let manureStatus = getActiveCategoryStatus(pData);
        let waterStatus = 'none';
        if (isWater && window.WaterMode && typeof window.WaterMode.resolveStatus === 'function') {
            waterStatus = window.WaterMode.resolveStatus(pData);
            color = window.WaterMode.getColor(waterStatus);
        } else if (isWater) {
            // WaterMode未読込時のフォールバック
            waterStatus = (pData.water_status === 'supplying') ? 'supplying'
                : (pData.water_status ? 'stopped' : 'none');
            color = waterStatus === 'supplying' ? '#2196F3'
                : (waterStatus === 'stopped' ? '#F44336' : '#9E9E9E');
        } else {
            color = STATUS_COLORS[manureStatus] || STATUS_COLORS['none'];
        }
        ...
        const handleFieldClick = () => {
            if (mapMode === 'water') {
                if (typeof openWaterStatusModal === 'function') openWaterStatusModal(pData);
                else if (window.WaterMode) window.WaterMode.openStatusModal(pData);
                return;
            }
            migratePDataManure(pData);
...
window.redrawMapForCurrentMode = function () {
    if (lastDrawnFieldData) {
        const keepFit = isFirstBoundsFit;
        isFirstBoundsFit = false;
        drawPolygons(lastDrawnFieldData);
        isFirstBoundsFit = keepFit;
        return;
    }
    const cached = localStorage.getItem('manureMapData');
    if (cached) {
...
window.setMapMode = function (mode) {
    const next = (mode === 'water') ? 'water' : 'prod';
    if (mapMode === 'water' && next === 'prod' && window.WaterMode) {
        window.WaterMode.deactivate();
    }
    applyMapModeChrome_(next);
    localStorage.setItem('mapUiMode', mapMode);

    const prodMenu = document.getElementById('prodFilterMenu');
    const waterMenu = document.getElementById('waterFilterMenu');
    if (prodMenu) prodMenu.style.display = 'none';
    if (waterMenu) waterMenu.style.display = 'none';

    window.redrawMapForCurrentMode();
    if (mapMode === 'water' && window.WaterMode) window.WaterMode.activate();
...
window.applyFilter = function() {
    if (mapMode === 'water') {
        if (typeof applyWaterFilter === 'function') applyWaterFilter();
        else if (window.WaterMode) window.WaterMode.applyFilter();
        return;
    }
    const menu = document.getElementById('prodFilterMenu') || document.getElementById('filterMenu');
    if (!menu) return;
...
window.toggleFilterMenu = function() {
    const menu = mapMode === 'water'
        ? document.getElementById('waterFilterMenu')
        : (document.getElementById('prodFilterMenu') || document.getElementById('filterMenu'));
    if (!menu) return;
...
window.calculateWeeklyWeatherOutlook = function() {
  ...
  if (forecastDays <= 0) {
    return { lastWeekAvg, recentAvg, weekDiff, forecastDays: 0, chips: [], summary: '今週の予報データがありません' };
  }
  ...
  return {
    lastWeekAvg, recentAvg, weekDiff, forecastDays, rainy, sunny, windy,
    tempTrend, tempTrendDiff, chips, summary: chips.map(c => c.text).join(' ／ ')
  };
};
...
window.renderWeeklyWeatherOutlookHtml = function() {
  ...
  if (o.weekDiff != null) {
    if (o.weekDiff > 0) weekDiffHtml = `<span style="color:#d32f2f;font-weight:bold;background:#ffebee;padding:3px 8px;border-radius:999px;">先週比 +${o.weekDiff}℃ 上がった</span>`;
    else if (o.weekDiff < 0) weekDiffHtml = `<span style="color:#1976d2;font-weight:bold;background:#e3f2fd;padding:3px 8px;border-radius:999px;">先週比 ${o.weekDiff}℃ 下がった</span>`;
    else weekDiffHtml = `<span style="color:#616161;font-weight:bold;background:#f0f0f0;padding:3px 8px;border-radius:999px;">先週比 ±0℃</span>`;
  }
