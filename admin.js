...
function ensureJpCitiesLoaded() {
    ...
    try {
        ...
        if (cached) {
            jpCitiesByPref = JSON.parse(cached);
            return Promise.resolve(jpCitiesByPref);
        }
    } catch (e) {}

    jpCitiesLoadPromise = fetch(JP_CITIES_API_URL)
        .then(res => {
            if (!res.ok) throw new Error('市区町村データの取得に失敗しました');
            return res.json();
        })
        .then(data => {
            jpCitiesByPref = data || {};
...
function syncLocationCityCustomVisibility(prefix) {
    ...
    if (citySel.value === 'custom') {
        ...
        if (citySel.value) cityCustom.value = '';
    }
}
...
window.onLocationPrefChange = function(sel) {
    ...
    ensureJpCitiesLoaded().then(() => {
        populateLocationCitySelect(prefix, sel.value, '');
    });
};
...
function renderInitData(data, opts) {
    ...
    if (!keepLocalMasters) {
        ...
        if (typeof window.refreshAdminMachineDisplayNames_ === 'function') {
            window.refreshAdminMachineDisplayNames_();
        }
...
window.addDetailWorkRow = (containerId, value = '', placeholder = '詳細作業名') => {
    const box = document.getElementById(containerId);
    if (!box) return;
    ...
    const safePh = String(placeholder || '詳細作業名').replace(/"/g, '&quot;');
    row.innerHTML = `<input type="text" class="form-input detail-work-input" style="flex:1; margin-bottom:0; padding:8px;" placeholder="${safePh}" value="${safeVal}"><button type="button" onclick="this.closest('.detail-work-row').remove()" style="background:#ffebee; color:#c62828; border:1px solid #ef9a9a; border-radius:4px; padding:8px 10px; font-weight:bold; cursor:pointer; flex-shrink:0;">×</button>`;
    box.appendChild(row);
    const input = row.querySelector('input');
    if (input && !value) input.focus();
};

window.buildDetailWorksEditorHtml = (containerId, detailWorksStr, placeholder = '詳細作業名') => {
    ...
    const rows = (items.length ? items : ['']).map((item, idx) => {
        ...
        return `<div class="detail-work-row" style="display:flex; gap:6px; align-items:center; margin-bottom:6px;">
            <input type="text" class="form-input detail-work-input" style="flex:1; margin-bottom:0; padding:8px;" placeholder="${safePh} ${idx + 1}" value="${safe}">
    ...
    return `<div id="${containerId}" style="background:#fafafa; border:1px solid #ddd; border-radius:6px; padding:8px; margin-bottom:8px;">${rows}</div>
        <button type="button" onclick="addDetailWorkRow('${containerId}', '', '${safePh}')" style="background:#e3f2fd; color:#1565c0; border:1px solid #90caf9; border-radius:4px; padding:6px 12px; font-size:12px; font-weight:bold; cursor:pointer; margin-bottom:10px;">＋ 追加</button>`;
};

window.getWorkMasterOptionsHtml = (selected) => {
    const selectedVal = String(selected || '').trim();
    let html = '<option value="">作業を選択...</option>';
    const names = (pdlWorkMaster || []).map(w => String((w && w.name) || w || '').trim()).filter(Boolean);
    const seen = {};
    names.forEach(name => {
        if (seen[name]) return;
        seen[name] = true;
...
window.addMachineWorkCategoryRow = (containerId, value = '') => {
    const box = document.getElementById(containerId);
    if (!box) return;
    const row = document.createElement('div');
    row.className = 'detail-work-row';
    row.style.cssText = 'display:flex; gap:6px; align-items:center; margin-bottom:6px;';
    row.innerHTML = `<select class="form-input detail-work-input" style="flex:1; margin-bottom:0; padding:8px;">${getWorkMasterOptionsHtml(value)}</select><button type="button" onclick="this.closest('.detail-work-row').remove()" style="background:#ffebee; color:#c62828; border:1px solid #ef9a9a; border-radius:4px; padding:8px 10px; font-weight:bold; cursor:pointer; flex-shrink:0;">×</button>`;
    box.appendChild(row);
};

window.buildMachineWorkCategoryEditorHtml = (containerId, workCategoryStr) => {
    const items = String(workCategoryStr || '').split(/[,、]/).map(s => s.trim()).filter(Boolean);
    const list = items.length ? items : [''];
    const rows = list.map(item => {
        return `<div class="detail-work-row" style="display:flex; gap:6px; align-items:center; margin-bottom:6px;">
            <select class="form-input detail-work-input" style="flex:1; margin-bottom:0; padding:8px;">${getWorkMasterOptionsHtml(item)}</select>
            <button type="button" onclick="this.closest('.detail-work-row').remove()" style="background:#ffebee; color:#c62828; border:1px solid #ef9a9a; border-radius:4px; padding:8px 10px; font-weight:bold; cursor:pointer; flex-shrink:0;">×</button>
        </div>`;
    }).join('');
    return `<div id="${containerId}" style="background:#fafafa; border:1px solid #ddd; border-radius:6px; padding:8px; margin-bottom:8px;">${rows}</div>
        <button type="button" onclick="addMachineWorkCategoryRow('${containerId}')" style="background:#e3f2fd; color:#1565c0; border:1px solid #90caf9; border-radius:4px; padding:6px 12px; font-size:12px; font-weight:bold; cursor:pointer; margin-bottom:10px;">＋ 作業を追加</button>`;
};

window.isMachineManageSign = (p) => {
...
window.getMachineTypeOptionsHtml = (selected) => {
    const list = pdlMachineTypes || [];
    let html = '<option value="">機械カテゴリを選択...</option>';
    list.forEach(t => {
...
window.openMasterDetail = (type, customEditHtml = null) => {
    ...
    if (customEditHtml) {
        ...
        if (type === 'crop') {
            ...
            formHtml += `<div style="display:flex; flex-direction:column; gap:8px;">
                ...
                <label style="font-size:12px; font-weight:bold; color:#555;">品目（必須）</label>
                <select id="add_container_crop" class="form-input" style="margin-bottom:0; padding:8px;"><option value="">選択してください</option>${cropOpts}</select>
                <div style="font-size:11px; color:#666;">※同じコンテナ種類でも品目ごとに別登録（内容個数が違うため共通はありません）</div>
            ...
            <div style="background:#e3f2fd; border:1px solid #90caf9; border-radius:8px; padding:12px;">
                ...
                <div id="pestCatalogStats" style="font-size:12px; color:#1565c0; font-weight:bold; margin-bottom:8px;">カタログ件数: 読込中...</div>
            ...
            <div style="margin-top:12px; background:#e8f5e9; border:1px solid #a5d6a7; border-radius:8px; padding:12px;">
                <div style="font-weight:bold; color:#2e7d32; margin-bottom:6px; font-size:14px;">② カタログから検索してマスタ登録</div>
                <div style="display:flex; gap:6px; flex-wrap:wrap; margin-bottom:8px;">
                  <input type="text" id="pestCatSearchQ" class="form-input" style="flex:1; min-width:120px; margin:0; padding:8px;" placeholder="農薬名・成分・登録番号">
                  <input type="text" id="pestCatSearchCrop" class="form-input" style="flex:1; min-width:100px; margin:0; padding:8px;" placeholder="作物名">
                </div>
                <button type="button" onclick="searchPesticideCatalogUi()" style="width:100%; background:#2e7d32; color:#fff; border:none; border-radius:4px; padding:10px; font-weight:bold; cursor:pointer; margin-bottom:8px;">検索</button>
                <div id="pestCatalogSearchResult" style="font-size:12px; color:#666;"></div>
            </div>`;
            ...
            <div style="margin-top:12px; background:#e8f5e9; border:1px solid #a5d6a7; border-radius:8px; padding:12px;">
                <div style="font-weight:bold; color:#2e7d32; margin-bottom:6px; font-size:14px;">② カタログから検索してマスタ登録</div>
                <div style="display:flex; gap:6px; flex-wrap:wrap; margin-bottom:8px;">
                  <input type="text" id="fertCatSearchQ" class="form-input" style="flex:1; min-width:120px; margin:0; padding:8px;" placeholder="肥料名・種類・登録番号">
                  <input type="text" id="fertCatSearchMaker" class="form-input" style="flex:1; min-width:100px; margin:0; padding:8px;" placeholder="メーカー">
                </div>
                <button type="button" onclick="searchFertilizerCatalogUi()" style="width:100%; background:#2e7d32; color:#fff; border:none; border-radius:4px; padding:10px; font-weight:bold; cursor:pointer; margin-bottom:8px;">検索</button>
            ...
            formHtml += `<div style="display:flex; flex-direction:column; gap:8px;">
                ...
                <label style="font-size:12px; font-weight:bold; color:#555;">型式名（最大2つ）</label>
                <div style="display:flex; gap:5px;">
                  <input type="text" id="add_mac_model" class="form-input" style="flex:1; margin-bottom:0; padding:8px;" placeholder="型式1（例：PH2R）" oninput="autoFillAdminMachineNumber_(true)">
                  <input type="text" id="add_mac_model2" class="form-input" style="flex:1; margin-bottom:0; padding:8px;" placeholder="型式2（任意）" oninput="autoFillAdminMachineNumber_(true)">
                </div>
    ...
    if (list.length === 0) {
        ...
        list.forEach(v => {
            let dispName = (type === 'cropCultSetting') ? (v.cropName || v.name || v) : (v.name || v);
            if (type === 'machine' && typeof window.buildAdminMachineDisplayName_ === 'function') {
                dispName = window.buildAdminMachineDisplayName_(
                    v.type,
                    v.machineNumber || v.serialNo,
                    v.model || v.modelType,
                    v.name || v,
                    v.group || v.mainCategory,
                    v.id,
                    v.model2
                ) || dispName;
            }
            ...
            if (type === 'container') {
                ...
                subInfo = `<span style="font-size:11px; background:#e8f5e9; color:#2e7d32; padding:2px 6px; border-radius:4px; margin-left:6px;">🌱 ${cropDisp}</span><span style="font-size:11px; background:#fff3e0; color:#e65100; padding:2px 6px; border-radius:4px; margin-left:4px;">中身: ${contentDisp}</span>`;
            }
            ...
            if (type === 'work') {
                ...
                const delPayload = encodeURIComponent(JSON.stringify({ name: v.name || dispName, crop: v.crop || (v.crops && v.crops[0]) || '' }));
                actionBtns = `<button onclick="openEditContainerMaster('${safeV}')" style="background:#e3f2fd; color:#1976d2; border:1px solid #90caf9; border-radius:4px; padding:4px 8px; font-weight:bold; font-size:12px; cursor:pointer; margin-right:4px;">✏️ 編集</button><button onclick="execMaster('${type}', 'delete', '${delPayload}')" style="background:#ffebee; color:#c62828; border:1px solid #ef9a9a; border-radius:4px; padding:4px 8px; font-weight:bold; font-size:12px; cursor:pointer;">× 削除</button>`;
            } else if (type === 'workCategory') {
...
window.openEditLocationMaster = (encodedStr) => {
    ...
    const editHtml = `
        <div style="background:#fff; padding:15px; border-radius:8px; border:1px solid #ddd; margin-bottom:15px;">
            ...
            <select id="edit_location_city" class="form-input" onchange="onLocationCityChange(this)">
...
window.openEditWorkCategoryMaster = (encodedName) => {
    ...
    const editHtml = `
        <div style="background:#fff; padding:15px; border-radius:8px; border:1px solid #ddd; margin-bottom:15px;">
            <h4 style="margin-top:0; color:#1976D2; font-size:15px; border-bottom:2px solid #1976D2; padding-bottom:5px;">✏️ 作業カテゴリの編集</h4>
            <input type="hidden" id="edit_workCategory_original_name" value="${safeName}">
            <label class="form-label">カテゴリ名</label>
            <input type="text" id="edit_workCategory_name" class="form-input" value="${safeName}">
            <div style="font-size:11px; color:#666; margin-top:6px;">※名前を変更すると、作業マスタ側の同じカテゴリもまとめて更新されます。</div>
            <div style="display:flex; gap:10px; margin-top:15px;">
                <button onclick="execMaster('workCategory', 'edit')" style="flex:1; background:#FF9800; color:white; border-radius:4px; border:none; padding:10px; font-weight:bold; cursor:pointer;">更新する</button>
                <button onclick="openMasterDetail('workCategory')" style="flex:1; background:#ccc; color:#333; border-radius:4px; border:none; padding:10px; font-weight:bold; cursor:pointer;">キャンセル</button>
            </div>
        </div>
    `;
...
window.openEditCropMaster = (encoded) => {
    ...
    const editHtml = `
        <div style="background:#fff; padding:15px; border-radius:8px; border:1px solid #ddd; margin-bottom:15px;">
            ...
            <div style="display:flex; gap:10px; margin-top:15px;">
                <button onclick="execMaster('crop', 'edit')" style="flex:1; background:#FF9800; color:white; border-radius:4px; border:none; padding:10px; font-weight:bold; cursor:pointer;">更新する</button>
                <button onclick="openMasterDetail('crop')" style="flex:1; background:#ccc; color:#333; border-radius:4px; border:none; padding:10px; font-weight:bold; cursor:pointer;">キャンセル</button>
            </div>
        </div>
    `;
...
window.refreshFertilizerCatalogStats = async function() {
    const el = document.getElementById('fertCatalogStats');
    if (!el) return;
    try {
        const res = await callGAS('getFertilizerCatalogStats', {});
        el.textContent = 'カタログ件数: ' + ((res && res.count) || 0).toLocaleString() + ' 件';
    } catch (e) {
        el.textContent = 'カタログ件数: 取得失敗';
    }
};

window.refreshPesticideCatalogStats = async function() {
    const el = document.getElementById('pestCatalogStats');
    if (!el) return;
    try {
        const res = await callGAS('getPesticideCatalogStats', {});
        el.textContent = 'カタログ件数: ' + ((res && res.count) || 0).toLocaleString() + ' 件';
    } catch (e) {
        el.textContent = 'カタログ件数: 取得失敗';
    }
...
window.buildFertilizerItemsFromCsvRows_ = function(csvRows) {
    const mapped = [];
    const seen = {};
    (csvRows || []).forEach(r => {
        ...
        if (!name) return;
        const item = {
            name, fertilizerType, nitrogen, phosphate, potash,
            components: extras.join('・'), volume: '', manufacturer, regNumber, note: ''
        };
        const key = (regNumber || name) + '\t' + (manufacturer || '');
        if (seen[key]) return;
        seen[key] = true;
        mapped.push(item);
    });
...
window.searchFertilizerCatalogUi = async function() {
    const area = document.getElementById('fertCatalogSearchResult');
    if (!area) return;
    const q = (document.getElementById('fertCatSearchQ')?.value || '').trim();
    const maker = (document.getElementById('fertCatSearchMaker')?.value || '').trim();
    if (!q && !maker) {
        customAlert('肥料名またはメーカーを入力してください');
        return;
    }
    ...
    try {
        ...
        const note = (res.truncated ? '（上位のみ表示）' : '') +
            ' ／ カタログ全体 ' + ((res.totalCatalog || 0).toLocaleString()) + ' 件';
        const rowsHtml = window._fertCatalogHits.map((it, idx) => {
            const npk = [it.nitrogen, it.phosphate, it.potash].filter(Boolean).join('-') || '-';
            const label = `${it.name} ／ ${it.manufacturer || '-'} ／ ${it.fertilizerType || '-'} ／ NPK ${npk}`;
            return `<label style="display:flex;gap:6px;align-items:flex-start;padding:4px 0;border-bottom:1px solid #c8e6c9;cursor:pointer;">
              <input type="checkbox" class="fertCatHitCheck" data-idx="${idx}" style="margin-top:3px;">
              <span style="line-height:1.35;">${String(label).replace(/</g, '&lt;')}</span>
            </label>`;
        }).join('');
        area.innerHTML =
            `<div style="font-weight:bold;color:#1b5e20;margin-bottom:6px;">${window._fertCatalogHits.length} 件 ${note}</div>` +
...
window.addSelectedFertilizerFromCatalog = async function() {
    const checks = Array.from(document.querySelectorAll('.fertCatHitCheck'));
    const rows = [];
    checks.forEach(cb => {
        if (!cb.checked) return;
        const idx = parseInt(cb.getAttribute('data-idx'), 10);
        const row = window._fertCatalogHits[idx];
        if (row) rows.push(row);
    });
    if (!rows.length) {
        customAlert('登録する行を選択してください');
    ...
    if (!await customConfirm(rows.length + ' 件を肥料マスタへ登録しますか？')) return;
    try {
        const res = await callGAS('importFertilizerMasterRows', { rows, userName: currentUser });
        pdlFertilizers = (res && res.fertilizers) || pdlFertilizers;
        localStorage.removeItem('pMapAdminInitData');
...
window.buildPesticideItemsFromFamicFiles_ = async function(baseFile, applyFiles, onProgress) {
    ...
    for (let i = 0; i < applyRows.length; i++) {
        ...
        const item = {
            ...
            cropName: window.findCsvCol_(r, ['作物名', '適用作物']),
            dilution: window.findCsvCol_(r, ['希釈倍数', '希釈倍率', '使用量']),
            phiDays: window.extractPhiDaysFromTiming_(useTimingText),
...
window.searchPesticideCatalogUi = async function() {
    const area = document.getElementById('pestCatalogSearchResult');
    if (!area) return;
    const q = (document.getElementById('pestCatSearchQ')?.value || '').trim();
    const crop = (document.getElementById('pestCatSearchCrop')?.value || '').trim();
    if (!q && !crop) {
        customAlert('農薬名または作物名を入力してください');
        return;
    }
    ...
    try {
        ...
        const note = (res.truncated ? '（上位のみ表示）' : '') +
            ' ／ カタログ全体 ' + ((res.totalCatalog || 0).toLocaleString()) + ' 件';
        const rowsHtml = window._pestCatalogHits.map((it, idx) => {
            const label = `${it.name} ／ ${it.cropName || '-'} ／ ${it.dilution || '-'} ／ ${it.phiDays !== '' && it.phiDays != null ? it.phiDays + '日前' : (it.useTimingText || '-')}`;
            return `<label style="display:flex;gap:6px;align-items:flex-start;padding:4px 0;border-bottom:1px solid #c8e6c9;cursor:pointer;">
              <input type="checkbox" class="pestCatHitCheck" data-idx="${idx}" style="margin-top:3px;">
              <span style="line-height:1.35;">${String(label).replace(/</g, '&lt;')}</span>
            </label>`;
        }).join('');
        area.innerHTML =
            `<div style="font-weight:bold;color:#1b5e20;margin-bottom:6px;">${window._pestCatalogHits.length} 件 ${note}</div>` +
            `<div style="max-height:220px;overflow:auto;background:#fff;border:1px solid #a5d6a7;border-radius:6px;padding:8px;margin-bottom:8px;">${rowsHtml}</div>` +
...
window.addSelectedPesticideFromCatalog = async function() {
    const checks = Array.from(document.querySelectorAll('.pestCatHitCheck'));
    const rows = [];
    checks.forEach(cb => {
        if (!cb.checked) return;
        const idx = parseInt(cb.getAttribute('data-idx'), 10);
        const row = window._pestCatalogHits[idx];
        if (row) rows.push(row);
    });
    if (!rows.length) {
        customAlert('登録する行を選択してください');
    ...
    if (!await customConfirm(rows.length + ' 件を農薬マスタへ登録しますか？')) return;
    try {
        const res = await callGAS('importPesticideMasterRows', { rows, userName: currentUser });
        pdlPesticides = (res && res.pesticides) || pdlPesticides;
        localStorage.removeItem('pMapAdminInitData');
...
window.openEditContainerMaster = (encoded) => {
    ...
    const editHtml = `
        <div style="background:#fff; padding:15px; border-radius:8px; border:1px solid #ddd; margin-bottom:15px;">
            ...
            <input type="hidden" id="edit_container_original_crop" value="${String(crop).replace(/"/g, '&quot;')}">
            ...
            <select id="edit_container_crop" class="form-input"><option value="">選択してください</option>${cropOpts}</select>
            <div style="font-size:11px; color:#666; margin-bottom:8px;">※共通設定はありません。品目ごとに内容個数を登録してください。</div>
            <div style="display:flex; gap:10px;">
                <div style="flex:1;">
...
window.openEditWorkMaster = (encodedStr) => {
    try {
        ...
        window.openWorkMasterEditModal(v);
    } catch(e) {
        console.error('openEditWorkMaster error:', e);
    }
};
...
window.openWorkMasterEditModal = (v) => {
    ...
    const safeName = window._escapeHtmlAdmin ? window._escapeHtmlAdmin(v.name) : v.name;
    const categories = (pdlWorkCategories && pdlWorkCategories.length) ? pdlWorkCategories : ["圃場作業", "事務作業", "保全・整備"];
    const catOpts = categories.map(c => `<option value="${c}" ${c === v.category ? 'selected' : ''}>${c}</option>`).join('');
    const selectedCrops = v.crops && v.crops.length ? v.crops : (v.cropName ? [v.cropName] : ['__common__']);

    let modal = document.getElementById('workMasterEditModal');
    if (!modal) {
        modal = document.createElement('div');
        modal.id = 'workMasterEditModal';
        modal.style.cssText = 'display:none; position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.6); z-index:15000; justify-content:center; align-items:center;';
        modal.onclick = (e) => { if (e.target === modal) closeWorkMasterEditModal(); };
        document.body.appendChild(modal);
    }

    const html = `
    <div style="background:#ffffff; width:92%; max-width:680px; max-height:85vh; border-radius:12px; box-shadow:0 10px 30px rgba(0,0,0,0.35); display:flex; flex-direction:column; overflow:hidden; animation: fadeIn 0.2s ease;">
      <!-- モーダルヘッダー -->
      <div style="background:#f8f9fa; border-bottom:1px solid #e0e0e0; padding:14px 18px; display:flex; justify-content:space-between; align-items:center;">
        <div style="font-size:16px; font-weight:bold; color:#1976d2; display:flex; align-items:center; gap:6px;">
          <span>✏️ 作業マスタの編集</span>
          <span style="font-size:13px; color:#666; font-weight:normal;">(${safeName})</span>
        </div>
        <button onclick="closeWorkMasterEditModal()" style="background:none; border:none; font-size:22px; color:#888; cursor:pointer; line-height:1; padding:0 4px;">&times;</button>
      </div>
      ...
      <div style="padding:20px; overflow-y:auto; flex:1;">
        ...
        <div style="margin-bottom:12px;">
          <label style="font-size:12px; font-weight:bold; color:#555; display:block; margin-bottom:4px;">作業カテゴリ</label>
          <select id="edit_work_category" class="form-input" style="width:100%; padding:9px; border-radius:6px; border:1px solid #ccc; font-size:14px;">${catOpts}</select>
        </div>
      ...
      <!-- モーダルフッター -->
      <div style="background:#f8f9fa; border-top:1px solid #e0e0e0; padding:12px 18px; display:flex; justify-content:flex-end; gap:10px;">
        <button onclick="closeWorkMasterEditModal()" style="background:#eee; color:#444; border:1px solid #ccc; border-radius:6px; padding:10px 18px; font-weight:bold; font-size:14px; cursor:pointer;">キャンセル</button>
        <button onclick="execMaster('work', 'edit')" style="background:#FF9800; color:white; border:none; border-radius:6px; padding:10px 24px; font-weight:bold; font-size:14px; cursor:pointer; box-shadow:0 2px 5px rgba(255,152,0,0.3);">更新保存する</button>
      </div>
    </div>
    `;
...
window.closeWorkMasterEditModal = () => {
    const modal = document.getElementById('workMasterEditModal');
    if (modal) modal.style.display = 'none';
};

window.openEditWorkMasterLegacy = (encodedStr) => {
    const v = JSON.parse(decodeURIComponent(encodedStr));
...
window.openEditMachineMaster = (encodedStr) => {
    ...
    const safeModel2 = String(v.model2 || "").replace(/"/g, '&quot;');
    ...
    const locOpts = '<option value="">拠点を選択...</option>' + (pdlLocations || []).map(l => {
        const sel = String(l) === String(v.location || '') ? 'selected' : '';
    ...
    const editHtml = `
        <div style="background:#fff; padding:15px; border-radius:8px; border:1px solid #ddd; margin-bottom:15px;">
            ...
            <label class="form-label">型式名（最大2つ）</label>
            <div style="display:flex; gap:5px; margin-bottom:10px;">
              <input type="text" id="edit_mac_model" class="form-input" style="flex:1; margin-bottom:0;" value="${safeModel}" placeholder="型式1">
              <input type="text" id="edit_mac_model2" class="form-input" style="flex:1; margin-bottom:0;" value="${safeModel2}" placeholder="型式2（任意）">
            </div>
...
window.buildAdminMachineDisplayName_ = (typeName, number, model, fallback, group, excludeId, model2) => {
    const g = String(group || '').trim();
    const typePart = String(typeName || '').trim();
    const modelPart = window.joinAdminMachineModels_(model, model2);
    const matched = (typeof window.findAdminMachinesMatchingTrio_ === 'function')
        ? window.findAdminMachinesMatchingTrio_(g, typePart, model, excludeId || null, model2)
        : [];
    // 自分以外が0台＝同型式は自分だけ → 番号非表示
    const omitNumber = matched.length === 0;
    if (window.MachineTaxonomy && typeof MachineTaxonomy.buildDisplayName === 'function') {
        return MachineTaxonomy.buildDisplayName('machine', typeName, number, modelPart, fallback || '', {
            group: g,
            omitNumber: omitNumber,
            forceShowNumber: !omitNumber
        }) || '';
    }
...
/** 同型式台数に応じて一覧の表示名を付け直す */
window.refreshAdminMachineDisplayNames_ = () => {
    (window.pdlMachines || []).forEach(m => {
        if (!m) return;
        m.name = window.buildAdminMachineDisplayName_(
            m.type,
            m.machineNumber || m.serialNo,
            m.model || m.modelType,
            m.name,
            m.group || m.mainCategory,
            m.id,
            m.model2
        ) || m.name;
    });
...
/** メインカテゴリ＋機械カテゴリ＋型式名（1・2）が一致する農機一覧 */
window.findAdminMachinesMatchingTrio_ = (group, typeName, model, excludeId, model2) => {
    const g = String(group || '').trim();
    const t = String(typeName || '').trim();
    const m = String(model || '').trim();
    const m2 = String(model2 || '').trim();
    return (window.pdlMachines || []).filter(mac => {
        ...
        return String(mac.group || '').trim() === g
            ...
            && String(mac.model2 || '').trim() === m2;
    });
};

/** 一致する既存台の最大番号+1（無ければ1） */
window.getNextAdminMachineNumber_ = (group, typeName, model, excludeId, model2) => {
    const matched = window.findAdminMachinesMatchingTrio_(group, typeName, model, excludeId, model2);
    let maxNum = 0;
    matched.forEach(mac => {
...
window.autoFillAdminMachineNumber_ = (force) => {
    ...
    const model2El = document.getElementById('add_mac_model2');
    const numEl = document.getElementById('add_mac_number');
    ...
    const model2 = model2El ? String(model2El.value || '').trim() : '';
    ...
    const matched = window.findAdminMachinesMatchingTrio_(group, typeName, model, null, model2);
    const next = window.getNextAdminMachineNumber_(group, typeName, model, null, model2);
...
window.execMaster = async (type, act, val) => {
    ...
    if (type === 'machine') {
        try {
            if (act === 'add') {
                ...
                const model2 = ((document.getElementById('add_mac_model2') || {}).value || '').trim();
                ...
                if (group && macType && model && !machineNumber) {
                    machineNumber = window.getNextAdminMachineNumber_(group, macType, model, null, model2);
                    const numEl = document.getElementById('add_mac_number');
                    if (numEl) {
                ...
                if (!macType && !model && !model2 && !machineNumber) {
                    customAlert("機械カテゴリ・型式名・番号のいずれかを入力してください");
                    return;
                }
                if (group && macType && model && machineNumber) {
                    const newNum = String(machineNumber).trim().replace(/^no\.?/i, '');
                    const dup = window.findAdminMachinesMatchingTrio_(group, macType, model, null, model2).some(mac => {
                        const existingNum = String(mac.machineNumber || mac.serialNo || '').trim().replace(/^no\.?/i, '');
                        return existingNum && newNum && existingNum === newNum;
                    });
                    if (dup) {
                        const suggested = window.getNextAdminMachineNumber_(group, macType, model, null, model2);
                        customAlert(`同じメインカテゴリ・機械カテゴリ・型式で番号「${machineNumber}」は既にあります。\n番号「${suggested}」などで登録してください。`);
                        return;
                    }
                }
                const name = window.buildAdminMachineDisplayName_(macType, machineNumber, model, macType || model || machineNumber, group, null, model2);
                ...
                const model2 = ((document.getElementById('edit_mac_model2') || {}).value || '').trim();
                ...
                if (group && macType && model && !number) {
                    number = window.getNextAdminMachineNumber_(group, macType, model, machineId, model2);
                }
                if (!macType && !model && !model2 && !number) {
                    customAlert("機械カテゴリ・型式名・番号のいずれかを入力してください");
                    return;
                }
                if (group && macType && model && number) {
                    const newNum = String(number).trim().replace(/^no\.?/i, '');
                    const dup = window.findAdminMachinesMatchingTrio_(group, macType, model, machineId, model2).some(mac => {
                        const existingNum = String(mac.machineNumber || mac.serialNo || '').trim().replace(/^no\.?/i, '');
                        return existingNum && newNum && existingNum === newNum;
                    });
                    if (dup) {
                        const suggested = window.getNextAdminMachineNumber_(group, macType, model, machineId, model2);
                        customAlert(`同じメインカテゴリ・機械カテゴリ・型式で番号「${number}」は既にあります。\n番号「${suggested}」などにしてください。`);
                        return;
                    }
                }
                const name = window.buildAdminMachineDisplayName_(macType, number, model, macType || model || number, group, machineId, model2);
    ...
    if (act === 'add') {
        if (type === 'crop') { const name = document.getElementById('add_crop_name').value.trim(); if (!name) { customAlert("作物名を入力してください"); return; } value = { name: name, density: parseInt(document.getElementById('add_crop_density').value || 0), tagAbbreviation: (document.getElementById('add_crop_tag_abbreviation')?.value || '').trim() }; }
            ...
            const crop = (document.getElementById('add_container_crop')?.value || '').trim();
        ...
        if (type === 'location') {
            ...
            const climates = getSelectedLocationClimates('edit');
            value = {
            ...
            if (newName !== String(originalName || "").trim() && (pdlMachineGroups || []).includes(newName)) {
                customAlert("既に登録されています");
                return;
            }
            value = {
            ...
            if (!newName) { customAlert("カテゴリ名を入力してください"); return; }
            if (newName !== String(originalName || "").trim() && (pdlWorkCategories || []).includes(newName)) {
                customAlert(`カテゴリ名「${newName}」は既に登録されています`);
                return;
            }
            value = {
                originalName: originalName,
            ...
            if (!newName) { customAlert("作物名を入力してください"); return; }
            if (newName !== String(originalName || "").trim() && (pdlCrops || []).some(c => c.name === newName)) {
                customAlert(`作物名「${newName}」は既に登録されています`);
                return;
            }
            value = {
                originalName: originalName,
            ...
            const originalCrop = document.getElementById('edit_container_original_crop')?.value || '';
            const newName = document.getElementById('edit_container_name').value.trim();
            const newCrop = (document.getElementById('edit_container_crop')?.value || '').trim();
            if (!newName) { customAlert("コンテナ種類を入力してください"); return; }
            if (!newCrop) { customAlert("品目を選択してください"); return; }
            if ((newName !== String(originalName || "").trim() || newCrop !== String(originalCrop || "").trim())
                && (pdlContainers || []).some(c => c.name === newName && c.crop === newCrop)) {
                customAlert(`「${newName}」×「${newCrop}」は既に登録されています`);
                return;
            }
        ...
        if (type === 'container') {
            let parsed = null;
            try { parsed = JSON.parse(decodeURIComponent(val)); } catch (e) { parsed = null; }
            const delName = parsed ? (parsed.name || '') : val;
            const delCrop = parsed ? (parsed.crop || '') : '';
            const msg = delCrop
              ? `コンテナ「${delName}」×品目「${delCrop}」を削除しますか？`
              : `削除しますか？`;
            if (!await customConfirm(msg)) return;
            value = { name: delName, crop: delCrop };
        } else {
            if (!await customConfirm(`削除しますか？`)) return;
            value = { id: val };
        }
...
function initMap() {
    ...
    if (!mapEl) {
        // 地図なしで resolve すると圃場描画が永久にスキップされるため、再試行のみ行う
        console.warn('地図コンテナ #map が見つかりません。再試行します');
...
window.resolvePrefectureFromMap = () => {
    return new Promise((resolve) => {
        ...
        new google.maps.Geocoder().geocode({ location: center }, (results, status) => {
            ...
            if (prefName) {
                window._fudePrefCache = { name: prefName, lat, lng };
            }
            resolve(prefName);
        });
    });
...
/**
 * 県内の筆ポリ JSON を並列取得してキャッシュする。
 * onFirstReady: 最初の1件が入った時点（表示可能）
 * onProgress(done, total)
 */
window.fetchFudeFilesParallel = async (regionData, options = {}) => {
    const onProgress = typeof options.onProgress === 'function' ? options.onProgress : () => {};
    const onFirstReady = typeof options.onFirstReady === 'function' ? options.onFirstReady : () => {};
    const files = (regionData && regionData.files) ? regionData.files.slice() : [];
    if (!files.length) return { loaded: 0, total: 0 };
    ...
    const queueRefresh = () => {
        ...
        requestAnimationFrame(() => {
            refreshQueued = false;
            if (typeof window.refreshFudeMapData === 'function') window.refreshFudeMapData(false);
        });
    };
    const markProgress = () => {
        done++;
    ...
    const n = Math.min(FUDE_FETCH_CONCURRENCY, Math.max(1, files.length));
    await Promise.all(Array.from({ length: n }, () => worker()));
    if (typeof window.refreshFudeMapData === 'function') window.refreshFudeMapData(true);
    return { loaded: done, total: files.length };
};

window.autoSwitchFudeRegion = async () => {
    if (window.isMapLoadingFude) return;

    const prefName = await window.resolvePrefectureFromMap();
...
window.preloadAllFudeDataSlowly = () => {
    ...
    function fetchNext() {
        ...
        if (!window.fudeCache[target.fileName]) {
            fetch(`${FUDE_R2_BASE_URL}/${target.folder}/${target.fileName}`)
                .then(res => res.json())
                .then(geoJson => {
                    window.fudeCache[target.fileName] = geoJson;
                    currentIndex++;
                    // バックグラウンド先読みは間隔を短めに（3秒→1.2秒）
    ...
    // アプリの起動が完全に終わって落ち着いた「3秒後」にひっそりとスタート！
...
document.getElementById('btnLoadFude').onclick = async () => {
    ...
    try {
        ...
        await window.fetchFudeFilesParallel(regionData, {
            onProgress: (d, t) => {
                btn.innerHTML = `⏳ ${prefName} ${d}/${t}`;
            },
            onFirstReady: () => {
                window.refreshFudeMapData(true);
                unlockUi();
                btn.innerHTML = `⏳ ${prefName} 追加読込中...`;
            }
        });
        window.refreshFudeMapData(true);
        unlockUi();
...
window.openM = (id) => {
    ...
    if (p.isMarker) h += `<button onclick="openCol('${id}')" style="flex:1;background:#f0f0f0;padding:8px 0;font-size:11px;border-radius:4px;color:#333;border:1px solid #ccc;white-space:nowrap;cursor:pointer;">アイコン変更</button>`;
    h += `</div>`;
...
window.handleStatusSelect = async (sel) => {
    if (sel.value === 'ADD_NEW') {
      ...
      // window.toggleTracking は tracking.js の共通モジュールを使用
      if (!window.toggleTracking) {
        window.toggleTracking = () => {
          if (typeof window.showClockModal === 'function') {
            const dt = { date: new Date().toISOString().split('T')[0], time: new Date().toTimeString().slice(0,5) };
