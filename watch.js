      ...
      try {
        ...
        // 💬 【通常/復元モード】テキスト指示の処理
          ...
          // 💬 【通常モード】AIによる自動修正ルート
            ...
            2. 【文字列置換の罠に注意】Nodeスクリプト等でファイルを書き換える際、改行（\\n）やクォーテーションのエスケープミスで、意図せず「\\n」という文字列そのものがファイルに書き込まれ、構文エラー（SyntaxError）を誘発する事故が多発しています。置換処理のコードは極めて慎重に記述してください。
            3. 【必須の検証・確認プロセス】書き換え後、いきなり完了合図を出さないでください。
               ・.jsファイルの場合: 必ず \`node -c ファイル名\` 等で構文チェックを行うこと。
               ・.htmlや.gsファイルの場合: 書き換えた部分周辺を \`Get-Content\` (Windows) や \`cat\` 等のコマンドでターミナルに出力し、「リテラル文字（\\nなど）がそのまま書き込まれていないか」「タグや括弧が壊れていないか」をあなた自身で厳格に目視確認すること。
            4. テストや目視確認で不備を見つけた場合は、自律的に「再修正 ➡️ 再確認」のループを繰り返してください。妥協は許されません。
            5. 作業用の一時スクリプトは、成功後に必ず削除してください。
            6. 全て完全に成功した場合のみ、「 .ai_task_done.txt 」を作成し、中に詳細な解説レポート（原因、修正内容、実施したテスト・確認内容）を書き込んでください。`;
