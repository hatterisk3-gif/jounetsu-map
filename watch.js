      ...
      try {
        ...
        // 💬 【通常/復元モード】テキスト指示の処理
          ...
          // 💬 【通常モード】AIによる自動修正ルート
            ...
            3. 作業中にファイル書き換えなどの目的で作成した一時スクリプトや画像ファイルは、システムに混入させないため、作業完了後に必ず削除コマンド（rm等）で消去してください。
            4. すべてのコード修正と不要ファイルの削除が終わってから、最後に必ず「 .ai_task_done.txt 」というファイルを作成・保存してください。その際、必ずファイルの中身に「エラーの原因と、具体的にどう修正したか」の詳細な解説レポート（あなたが今考えたこと）をテキストで書き込んでください。これがそのまま報告メールになります。`;
