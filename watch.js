      ...
      try {
        ...
        // 💬 【通常/復元モード】テキスト指示の処理
          ...
          // 💬 【通常モード】AIによる自動修正ルート
            ...
            🚨【絶対ルール（厳守）】
            1. 【直接編集＆自律テスト】IDEのDiff/Review機能は使用禁止。Node.js等でファイルを直接上書き保存し、直後に必ず \`node -c\` やターミナル出力で「構文エラー」や「\\n等のエスケープ漏れ」がないか自律的にテスト・再修正すること。
            2. 作業に使用した一時スクリプトや、システム動作に関係のないファイルはテスト成功後に必ず削除すること。
            3. 【完了報告（文字化け厳禁）】全て完了後、詳細な解説レポートを記した「 .ai_task_done.txt 」を作成すること。ただしPowerShellのコマンドは文字化けするため使用禁止。必ず Node.js の \`fs.writeFileSync('.ai_task_done.txt', 'レポート本文', 'utf8')\` を使って保存すること。`;
