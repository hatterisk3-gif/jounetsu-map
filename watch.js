      ...
      try {
        ...
        // 💬 【通常/復元モード】テキスト指示の処理
          ...
          // 💬 【通常モード】AIによる自動修正ルート
            ...
            // 🌟 AIへの指示：HTML編集時の罠（エスケープミス）対策と目視確認を義務化！
            // 🌟 AIへの指示：Windowsの文字化け対策（UTF-8保存の強制）を追加！
            const modifyPrompt = `${imageContext}${cleanCommand}。
            🚨【絶対ルール】
            1. エディタのファイル編集機能（DiffやReview、Accept待ち状態）は絶対に使用せず、必ずターミナルツール（nodeスクリプト等）で直接ファイルを上書き保存してください。
            2. 【文字列置換の罠に注意】Nodeスクリプト等でファイルを書き換える際、改行（\\n）やエスケープミスで、意図せず「\\n」という文字列そのものがファイルに書き込まれ、構文エラーを誘発する事故が多発しています。置換コードは極めて慎重に記述してください。
            3. 【必須の検証・確認プロセス】書き換え後、いきなり完了合図を出さないでください。
               ・.jsファイルの場合: 必ず \`node -c ファイル名\` 等で構文チェックを行うこと。
               ・.htmlや.gsファイルの場合: 書き換えた部分周辺をターミナルに出力し、「リテラル文字（\\nなど）がそのまま書き込まれていないか」「タグが壊れていないか」をあなた自身で厳格に目視確認すること。
            4. テストや目視確認で不備を見つけた場合は、自律的に「再修正 ➡️ 再確認」のループを繰り返してください。
            5. 作業用の一時スクリプトは、成功後に必ず削除してください。
            6. 【重要：文字化け対策】全て完全に成功した場合のみ、「 .ai_task_done.txt 」を作成し、中に詳細な解説レポートを書き込んでください。その際、PowerShellのechoやリダイレクト(>)を使用するとUTF-16になり文字化けするため絶対に使用禁止です。必ず Node.js の \`fs.writeFileSync('.ai_task_done.txt', 'レポート本文', 'utf8')\` を実行する形で、確実な【UTF-8】エンコーディングで保存してください。`;
